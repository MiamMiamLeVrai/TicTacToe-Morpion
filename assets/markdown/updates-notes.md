...
- The `<decoding=async>` tag has been changed to `<decoding=sync>` for all images on the site to improve image loading performance and prevent display issues.
...
- `credits.md` has been updated to include English language options and additions regarding the rights to use site assets.
...
- Changement de la balise `<decoding=async>` en `<decoding=sync>` pour toutes les images du site, afin d'améliorer les performances de chargement des images et éviter les problèmes d'affichage.
...
1. For `index.html`, minor corrections to the file, particularly on the choice of available languages, to better fit screen readers and HTML semantics,
2. Minors corrections in some of the HTML files,
3. Updated the version number from 1.1.1 to 1.2.0.

CSS3 Files (SCSS)

1. For `index.scss`, light reworks on the design of the choices of available languages, allowing better readability and more easily adding other available languages in the next updates,
2. For `homepage.scss`, light layouts of some elements and HTML design for better readability,
3. For `download.scss`, additions and corrections of features for screen readers and keyboard navigation,
4. Minor corrections in the SCSS files.

JavaScript Files

1. For `animation.js`, new animations for some buttons on the site, including the navigation buttons "Go to content" and "Go to Get elements",
2. Rewriting the code to better adapt to multiple HTML files,
3. For `tictactoe-en.js`, added new animations for some elements of the game.

Others Things

1. The `draft` file has been removed, as it was not used and did not add value to the site,
2. Minor corrections in the `README.md` file,
3. Minor corrections in the `site.webmanifest` file,
4. Minor additions in the `.gitignore` and `robots.txt` files,
5. `updates-notes.md` will only present new versions for better understanding.

🇫🇷 Notes des mise à jour
⚙️ NOUVELLE VERSION : Version 1.2.0 (27/02/2026)
C'est quoi les nouveautés 👀 ?
HTML

1. Pour `index.html`, petite réécriture du fichier, notamment sur le choix des langues disponibles, pour mieux coller aux lecteurs d'écrans et à la sémantique HTML,
2. Corrections mineures dans certains des fichiers HTML,
3. Version mise à jour de la version 1.1.1 vers 1.2.0.

CSS3 (SCSS)

1. Pour `index.scss`, léger retravaille sur le design des choix des langues disponibles, permettant une meilleure lisibilité et de rajouter plus facilement d'autres langues disponibles dans de prochaines mises à jour,
2. Pour `homepage.scss`, léger agencements de certains éléments et design HTML pour une meilleure lisibilté,
3. Pour `download.scss`, ajouts et correctifs de fonctionnalités pour les lecteurs d'écrans et navigations par claviers,
4. Correctifs mineurs dans les fichiers SCSS.

JavaScript

1. Pour `animation.js`, nouveaux animations pour certains boutons du site, notamment les boutons de navigations "Aller au contenu" et "Aller sur __Obtenir les éléments__",
2. Réécriture du code pour mieux s'adapter multiples fichiers HTML,
3. Pour `tictactoe-fr.html`, ajout de nouvelles animations pour certains éléments du jeu.

Autres

1. Le dossier `draft` a été supprimé, car il n'était pas utilisé et n'apportait pas de valeur ajoutée pour le site,
2. Modifications mineures dans le fichier `README.md`,
3. Modifications mineurs dans le fichier `site.webmanifest`,
4. Ajouts mineures dans les fichiers `.gitignore` et `robots.txt`,
5. `updates-notes.md` présentera uniquement les nouvelles versions pour une meilleure compréhension.

AMUSEZ-VOUS BIEN SUR LE MORPION !

🇺🇸 Update Notes
⚙️ NEW VERSION: Version 1.1.0 (2026/02/17)
What's new 👀?
HTML Files

1. Removal of unnecessary HTML elements (mainly `choice-modes-fr.html`, the others considered minor),
2. New custom HTML error page `404.html` (more in line with the site's theme),
3. Addition of some HTML elements + improved consistency between HTML files,
4. New visible feature (_but not for now…_ 😏).

CSS3 Files (SCSS)

1. Reorganization of classes and SCSS tags (organized according to the order of appearance of classes, IDs, and HTML tags),
2. Minor optimization of SCSS stylesheets,
3. Added accessibility features to reduce animation size,
4. Small addition of animation of the page `tictactoe-en.html`.

JavaScript Files

1. Removal of unnecessary JavaScript files,
2. New JavaScript file named `animation.js` (a file containing all the site's animations),
3. Separation of the Tic-Tac-Toe game logic from the animations.

Other Things

1. Minor rework of the site's images,
2. Replacement of the old assets of the `download` folder with the new assets,
3. Small addition on `robots.txt`.

⚙️ NEW VERSION: Version 1.0.1 (2026/02/12)
What's new 👀?

1. Corrected links in metadatas for sharing from other sites (Facebook, LinkedIn, etc.),
2. `updates-notes.md` is now available in English,
3. Small modifications in `README.md`.

⚙️ 1.0.0 Version (2026/02/12)
Details

1. Option to choose between two languages (French and English),
2. All web pages translated into English,
3. Compatibility only with recent browsers,
4. Basic features in `tictactoe-fr.html` (random draw, restart a game, etc.).

🇫🇷 Notes des mise à jour
⚙️ NOUVELLE VERSION : Version 1.1.0 (17/02/2026)
...
1. Réorganisation des classes + balises SCSS (organisé par rapport à l'ordre d'appararition des classes, id et balises HTML),
2. Légère optimisation des stylesheets SCSS,
3. Ajout d'une accessibilité permettant de réduire les animations,
4. Petit ajout d'animation pour la page `tictactoe-fr.html`.

JavaScript

1. Suppression des fichiers JavaScript en trop,
2. Nouveau fichier JavaScript nommée `animation.js` (fichier qui réunit toutes les animations du site),
3. Séparation de la logique du Morpion et des animations.

Autres

1. Léger retravail sur les images du site,
2. Remplacement des anciens assets du dossier `download` par les nouveaux assets,
3. Léger ajout dans `robots.txt`.

⚙️ NOUVELLE VERSION : Version 1.0.1 (12/02/2026)
C'est quoi les nouveautés 👀 ?

1. Corrections de liens dans les metadonnées permettant le partage d'autres sites (Facebook, LinkedIn, etc.),
2. `updates-notes.md` disponible en anglais,
3. Légère modifications dans `README.md`.

⚙️ Version 1.0.0 (12/02/2026)
Détails

1. Possibilités de choisir entre deux langues (Français et Anglais),
2. Pages webs toutes traduites en anglais,
3. Compatibilité uniquement avec les récents navigateurs,
4. Fonctionnalités basiques dans `tictactoe-fr.html` (tirage au sort, recommencer une partie, etc.).
